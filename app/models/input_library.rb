...
class InputLibrary < ActiveRecord::Base
  ...
  def self.get_github_item_id_from_gem_name(gem_name)
    InputLibrary.where(name: gem_name)
    .first
    .try(:input_project)
    .try(:github_item_id)
  end
  ...
  def self.get_full_name_from_gem_name(gem_name)
    ...
    .where(InputLibrary.arel_table[:homepage_uri].matches("%/#{gem_name}"))
    .first
    .try(:homepage_uri)

    if full_name.nil?
      full_name = InputLibrary.where(InputLibrary.arel_table[:source_code_uri].matches("%/github.com/%"))
      .where(InputLibrary.arel_table[:source_code_uri].matches("%/#{gem_name}"))
      .first
      .try(:source_code_uri)
    end
